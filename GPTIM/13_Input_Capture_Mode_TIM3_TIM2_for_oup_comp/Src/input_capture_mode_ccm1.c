...
void input_capture_mode_pa6(void)
{
...
/*2.To make the PA6 as the Alternate function Register */

	GPIOA->MODER = ((GPIOA->MODER & ~(1U<<12)) | (1U<<13));

/*3.To set the mode of Alternate function register*/

	GPIOA->AFR[0] = ((GPIOA->AFR[0] & ~(13U<<24)) | (1U<<25));

	/*For the Values to be in TIMER*/

/*1.TO GIVE CLOCK SOURCE TO APB1BUS*/

	RCC->APB1ENR |= GPTIM3_ENR;
...
/*1.Set the compare mode register in input mode in channel 1 CH1 Capture/Compare 1 selection */

	TIM3->CCMR1 = ((TIM3->CCMR1 & ~(1U<<1)) | CC1S);
